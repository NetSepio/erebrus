...
install_dependencies() {
    ...
    if is_docker_installed; then
        ...
        if command -v apt-get > /dev/null; then
	     (sh -c 'sudo apt-get update && sudo apt-get install -y containerd docker.io && sudo apt-get install netcat-* -y && sudo apt-get install lsof -y && sudo systemctl start docker && sudo systemctl enable docker' > /dev/null 2> error.log) &
	    ...
	    show_spinner $!
            ...
            (sh -c 'sudo yum install yum-utils -y && sudo yum install nmap-ncat* -y && sudo yum install lsof -y && sudo yum-config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo && yum install -y docker && sudo systemctl start docker && sudo systemctl enable docker' > /dev/null 2> error.log) &
            if ! check_docker_status; then
                echo "Starting docker......."
                sleep 10
                sudo systemctl daemon-reload
	    ...
	    show_spinner $!
            ...
            (sh -c 'sudo pacman -Sy --noconfirm docker && sudo systemctl start docker && sudo systemctl enable docker' > /dev/null 2> error.log) &
            if ! check_docker_status; then
                echo "Starting docker......."
                sleep 10
                sudo systemctl daemon-reload
	    ...
	    show_spinner $!
            ...
            (sh -c 'sudo dnf install dnf-plugins-core && dnf config-manager --add-repo https://download.docker.com/linux/fedora/docker-ce.repo && dnf install -y docker-ce docker-ce-cli containerd.io && sudo systemctl start docker && sudo systemctl enable docker' > /dev/null 2> error.log) &
            if ! check_docker_status; then
                echo "Starting docker......."
                sleep 10
                sudo systemctl daemon-reload
...
configure_node() {
    ...
    while true; do
	    ...
	    INSTALL_DIR=${SNAP_USER_DATA}
	fi
	printf "Data directory is set to '%s'" "$INSTALL_DIR"
